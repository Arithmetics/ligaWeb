    ...
    csv()
      ...
      .on("record", function(row, index) {
        ...
        if (options.sample) {
          ...
          if (
            !row.origin_name.match(test) ||
            !row.destination_name.match(test)
          ) {
            return;
          }
        }
        ...
        data.regions[row.originregion_name] =
          data.regions[row.originregion_name] || [];
        if (
          data.regions[row.originregion_name].indexOf(row.origin_name) === -1
        ) {
          data.regions[row.originregion_name].push(row.origin_name);
        }

        // collect migration data
        data.migrations[row.origin_name] =
          data.migrations[row.origin_name] || {};
        // country to country
        data.migrations[row.origin_name][row.destination_name] =
          data.migrations[row.origin_name][row.destination_name] || {};
        // country to region
        data.migrations[row.origin_name][row.destinationregion_name] =
          data.migrations[row.origin_name][row.destinationregion_name] || {};
        data.migrations[row.originregion_name] =
          data.migrations[row.originregion_name] || {};
        // region to country
        data.migrations[row.originregion_name][row.destination_name] =
          data.migrations[row.originregion_name][row.destination_name] || {};
        // region to region
        data.migrations[row.originregion_name][row.destinationregion_name] =
          data.migrations[row.originregion_name][row.destinationregion_name] ||
          {};
        ...
        years.forEach(function(year) {
          ...
          // country to region
          data.migrations[row.origin_name][row.destinationregion_name][year] =
            data.migrations[row.origin_name][row.destinationregion_name][
              year
            ] || 0;
          data.migrations[row.origin_name][row.destinationregion_name][
          ...
          // region to country
          data.migrations[row.originregion_name][row.destination_name][year] =
            data.migrations[row.originregion_name][row.destination_name][
              year
            ] || 0;
          data.migrations[row.originregion_name][row.destination_name][
      ...
      .on("end", function() {
        var keys = grunt.util._.union(
          ...
          function(memo, region) {
            ...
            memo.keys = memo.keys.concat(
              data.regions[region] && data.regions[region].sort()
            );
            return memo;
          },
          { indices: [], keys: [] }
      ...
      file.src.forEach(function(src) {
        ...
        compile(src, options, function(err, data) {
          if (err) {
            ...
            grunt.file.write(
              file.dest,
              JSON.stringify(data, null, options.sample ? 2 : 0)
            );
          }
          done(!err);
        });
